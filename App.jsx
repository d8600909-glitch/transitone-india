...
const BUSES = [
  {id:"GJ05BT1234",route:"101",driver:"Rajesh Kumar",  conductor:"Mohan Lal",  speed:42,stop:"Udhna Darwaja",  next:"Surat Railway St.",  delay:0,occupancy:67,fuel:"Electric",battery:78, status:"On Time",rating:4.8,city:"Surat",    fitness:"Valid till Dec 2025",insurance:"Valid till Mar 2026"},
  {id:"MH01AB5678",route:"303",driver:"Suresh Patil",  conductor:"Priya Nair", speed:38,stop:"Andheri East",   next:"Kurla West",          delay:3,occupancy:89,fuel:"Diesel",  battery:null,status:"Delayed", rating:4.2,city:"Mumbai",   fitness:"Valid till Oct 2025",insurance:"Valid till Jan 2026"},
  {id:"DL03CD9012",route:"404",driver:"Amit Singh",    conductor:"Ritu Sharma",speed:55,stop:"Connaught Place",next:"India Gate",           delay:0,occupancy:45,fuel:"CNG",     battery:null,status:"On Time",rating:4.9,city:"Delhi",    fitness:"Valid till Feb 2026",insurance:"Valid till Jun 2026"},
  {id:"KA01EF3456",route:"505",driver:"Venkat Rao",    conductor:"Lakshmi V",  speed:31,stop:"MG Road",        next:"Indiranagar",         delay:5,occupancy:92,fuel:"Electric",battery:61, status:"Delayed", rating:4.5,city:"Bangalore",fitness:"Valid till Nov 2025",insurance:"Valid till Apr 2026"},
  {id:"GJ01GH7890",route:"202",driver:"Dhruv Patel",   conductor:"Kavita Shah",speed:47,stop:"BRTS Stand",     next:"Paldi",               delay:0,occupancy:34,fuel:"Electric",battery:95, status:"On Time",rating:4.7,city:"Ahmedabad",fitness:"Valid till Jan 2026",insurance:"Valid till May 2026"},
  {id:"MH02CD3344",route:"1010",driver:"Sanjay More",  conductor:"Anita Desai",speed:29,stop:"Bandra Stn",     next:"Khar Road",           delay:2,occupancy:78,fuel:"Diesel",  battery:null,status:"Delayed", rating:4.3,city:"Mumbai",   fitness:"Valid till Sep 2025",insurance:"Valid till Dec 2025"},
];
...
const STOPS = [
  {id:"S01",name:"Surat Railway Station",city:"Surat",    routes:["101","202","303"],lat:21.206,lng:72.836,facilities:["Shelter","Seating","Display Board"]},
  {id:"S02",name:"Andheri Metro",        city:"Mumbai",   routes:["303","1010"],     lat:19.119,lng:72.848,facilities:["Shelter","Seating","Display Board","Lift"]},
  {id:"S03",name:"Connaught Place",      city:"Delhi",    routes:["404","808"],       lat:28.632,lng:77.219,facilities:["Shelter","Seating","Display Board","Drinking Water"]},
  {id:"S04",name:"MG Road",             city:"Bangalore",routes:["505","909"],       lat:12.975,lng:77.607,facilities:["Shelter","Seating"]},
  {id:"S05",name:"BRTS Paldi",          city:"Ahmedabad",routes:["202","1111"],      lat:23.030,lng:72.587,facilities:["Shelter","Seating","Display Board","Toilet"]},
  {id:"S06",name:"Hitech City",         city:"Hyderabad",routes:["606"],             lat:17.445,lng:78.380,facilities:["Shelter","Seating","Drinking Water"]},
];
...
{"fastest":{"time":"string","fare":"string","transfers":0,"walk":"string","mode":"string","steps":["s1","s2","s3"],"delay":"On Time"},"cheapest":{"time":"string","fare":"string","transfers":0,"walk":"string","mode":"string","steps":["s1","s2","s3"],"delay":"On Time"},"leastWalk":{"time":"string","fare":"string","transfers":0,"walk":"string","mode":"string","steps":["s1","s2","s3"],"delay":"On Time"},"wheelchair":{"time":"string","fare":"string","transfers":0,"walk":"string","mode":"string","steps":["s1","s2","s3"],"delay":"On Time"},"tip":"string","crowdAlert":"string"}`}]
  ...
  return (
    <>
      ...
      {/* ══════════ SIGN IN MODAL ══════════ */}
      {signModal && (
        <div className="modal-bg" onClick={()=>{setSignModal(false);setSignDone(false);setSignErr("");}}>
          <div className="modal" style={{maxWidth:420}} onClick={e=>e.stopPropagation()}>
            <div className="modal-x" onClick={()=>{setSignModal(false);setSignDone(false);setSignErr("");}}>✕</div>
            {!signDone ? (<>
              <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:20,fontWeight:800,marginBottom:6}}>
                {signTab==="login"?"Welcome Back 👋":"Create Account 🚌"}
              </div>
              <div style={{fontSize:12.5,color:"var(--t2)",marginBottom:20}}>
                {signTab==="login"?"Sign in to access your passes, trips and favourites.":"Join TransitOne India — free forever."}
              </div>
              {/* Tab toggle */}
              <div style={{display:"flex",gap:0,marginBottom:20,background:"var(--g1)",borderRadius:"var(--r2)",border:"1px solid var(--b1)",overflow:"hidden"}}>
                {["login","register"].map(t=>(
                  <button key={t} onClick={()=>{setSignTab(t);setSignErr("");}} style={{flex:1,padding:"10px 0",border:"none",background:signTab===t?"var(--grad)":"transparent",color:signTab===t?"#fff":"var(--t2)",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Inter',sans-serif",transition:"all .2s"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:11,marginBottom:16}}>
                {signTab==="register" && (
                  <input className="inp" placeholder="Full Name" value={signName} onChange={e=>setSignName(e.target.value)}/>
                )}
                <input className="inp" type="email" placeholder="Email address" value={signEmail} onChange={e=>setSignEmail(e.target.value)}/>
                {signTab==="register" && (
                  <input className="inp" placeholder="Mobile number (10 digits)" maxLength={10} value={signPhone} onChange={e=>setSignPhone(e.target.value.replace(/\D/g,""))}/>
                )}
                <input className="inp" type="password" placeholder="Password" value={signPass} onChange={e=>setSignPass(e.target.value)}
              ...
              {/* Social sign-in */}
              <div style={{textAlign:"center",margin:"14px 0",fontSize:12,color:"var(--t3)"}}>— or continue with —</div>
              <div style={{display:"flex",gap:9}}>
                {[["🔵","Google"],["⬛","Apple"],["📱","OTP"]].map(([ico,lbl])=>(
                  <button key={lbl} onClick={()=>{setSignDone(true);}} style={{flex:1,padding:"9px 0",borderRadius:"var(--r2)",background:"var(--g2)",border:"1px solid var(--b2)",color:"var(--t2)",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>
                    {ico} {lbl}
                  </button>
                ))}
              </div>
              {signTab==="login" && (
                <div style={{textAlign:"center",marginTop:12,fontSize:12,color:"var(--t3)"}}>
                  Don't have an account?{" "}
                  <span style={{color:"var(--a2)",cursor:"pointer",fontWeight:600}} onClick={()=>setSignTab("register")}>Register free →</span>
                </div>
              )}
            </>) : (
              <div style={{textAlign:"center",padding:"20px 10px"}}>
                <div style={{fontSize:56,marginBottom:14,animation:"pop .4s ease"}}>✅</div>
                <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:20,fontWeight:800,marginBottom:8}}>
                  {signTab==="login" ? "Signed In!" : "Account Created!"}
                </div>
                <div style={{color:"var(--t2)",fontSize:13,marginBottom:20}}>
                  Welcome to TransitOne India, <strong style={{color:"var(--a2)"}}>{signName||signEmail.split("@")[0]||"Traveller"}</strong>!
                </div>
                <button className="pbtn" onClick={()=>{setSignModal(false);setSignDone(false);nav("Passenger");}}>
                  📊 Go to My Dashboard
                </button>
              </div>
            )}
      ...
      {tab==="Home"&&<>
        <div className="hero">
          ...
          <div className="scard">
            <div className="irow">
              <input className="inp" placeholder="📍 From — stop, landmark, hospital, college…" value={from} onChange={e=>setFrom(e.target.value)}/>
              <input className="inp" placeholder="🎯 To — destination" value={to} onChange={e=>setTo(e.target.value)}/>
            </div>
            <div className="irow">
      ...
      {tab==="Journey"&&(
        <div className="sec">
          ...
          <div className="scard" style={{maxWidth:"100%",marginBottom:18}}>
            <div className="irow">
              <input className="inp" placeholder="📍 From — stop, hospital, college, mall, landmark…" value={from} onChange={e=>setFrom(e.target.value)}/>
              <input className="inp" placeholder="🎯 To — destination" value={to} onChange={e=>setTo(e.target.value)}/>
            </div>
            <div className="irow">
